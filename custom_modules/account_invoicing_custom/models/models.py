...
class AccountMoveLine(models.Model):
    ...
    def _get_price_total_and_subtotal_model(self, price_unit, quantity, discount, currency, product, partner, taxes, move_type):
        ...
        #        if sale_line.product_id.name == self.product.name:
        #            # Compute 'price_subtotal'.
        #            price_unit_wo_discount = sale_line.price_subtotal
        ...
        # Compute 'price_subtotal'. -> original
        price_unit_wo_discount = price_unit * (1 - (discount / 100.0))
        subtotal = quantity * price_unit_wo_discount
        
        for sale_line in self.sale_line_ids:
            if sale_line.product_id.name == self.product_id.name:
                # Compute 'price_subtotal'.
                price_unit_wo_discount = sale_line.price_subtotal
                subtotal = quantity * price_unit_wo_discount
